...
const AnimatedText = ({ baseOpacity, text }: Props) => {
  return (
    <>
      ...
      <motion.h1
        className='text-white blur-md text-9xl select-none'
      >
        {text.split("").map((char, index) => (
          <motion.span
            key={index}
...
function App() {
  ...
  return (
    <ParallaxProvider>
      ...
      initial={{ backdropFilter: 'blur(0vw)' }}
      animate={{ backdropFilter: backgroundBlur ? 'blur(1vw)' : 'blur(0vw)' }}
    ...
    <div className='bg-black h-[400vh] overflow-x-hidden w-screen flex items-center flex-col justify-start'>
      ...
      translateX={["100%", "0%"]}
      >
      <div className='flex items-center w-screen h-screen justify-center flex-col z-50'>
        <div className='flex items-center justify-center flex-col boxdesign w-[80%] h-[80%]'>
